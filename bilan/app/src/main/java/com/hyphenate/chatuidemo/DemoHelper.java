...
//            showToast("onMemberExited: " + member);
    ...
    private EaseUser getUserInfo(String username) {
        ...
        if (userBeanMap.containsKey(username)){
            UserBean userBean = getUserBeanForUid(username);
            if (userBean != null) {
...
//            Log.d("username",username+"加载资料");
    ...
    public Map<String,UserBean> userBeanMap = new HashMap<>();//
    public Map<String,String> userIdMap = new HashMap<>();//

    public UserBean getUserBeanForUid(String username){
    ...
    public void getUserBean(final String username,boolean isUpDate,final UserInfoListener l) {
        ...
        final String uid = username.substring(5, username.length());
        api.uid = uid;
        httpClient.request(api, new BeanRequest.SuccessListener<DataBean<UserBean>>() {
            ...
            public void onResponse(DataBean<UserBean> response) {
                ...
                if (StringUtils.isSame(uid, SharedAccount.getInstance(appContext).getServiceId())){
